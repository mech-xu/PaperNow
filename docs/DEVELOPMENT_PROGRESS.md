...
## 持续优化 (2026-05-03 ~ 2026-05-04)

### O1: 搜索源扩展 — ✅ 完成

| 子任务 | 状态 | 说明 |
|--------|------|------|
| 5 搜索源实现 | ✅ | arXiv, PubMed, ChinaRxiv, bioRxiv, medRxiv |
| 搜索页标题显示源名 | ✅ | "Search - medRxiv" 等 |
| 各页面返回按钮 | ✅ | ‹ 按钮返回首页 |
...
| 子任务 | 状态 | 说明 |
|--------|------|------|
| documents.id UUID→TEXT | ✅ | 迁移支持外部 ID 如 "medrxiv:10.1101/..." |
| RLS INSERT/UPDATE 策略 | ✅ | 认证用户可插入/更新 documents |
| 409 Conflict 处理 | ✅ | user_collections UNIQUE 约束，catch 23505 |
| 收藏按钮样式 | ✅ | 未收藏: 白底蓝字，已收藏: 蓝底白字 |
| PubMed 日期格式修复 | ✅ | sanitizeDate(): "2026 Apr" → "2026-04-01" |
| 详情页 Supabase 回退 | ✅ | searchStore → localStorage → Supabase |

### O3: 收藏页面精简 — ✅ 完成

- 移除阅读状态筛选、标签筛选栏、标签对话框
- 749 行 → ~170 行

### O4: bioRxiv/medRxiv 搜索优化 — ✅ 完成

| 子任务 | 状态 | 说明 |
|--------|------|------|
| DOI 去重 | ✅ | deduplicateByDoi() 保留最新版本 |
| 多页获取 | ✅ | 最多 10 页 (300 篇论文) |
| 结果去重 | ✅ | searchStore 按 ID 去重 |
| 加载抖动修复 | ✅ | 稳定 loading-initial 状态 |
| 默认日期范围 2 个月 | ✅ | 3x 减少页面获取量 |

### O5: 代理缓存 (L1+L2) — ✅ 完成

| 子任务 | 状态 | 说明 |
|--------|------|------|
| L1: Cache API | ✅ | 数据中心本地缓存，<10ms 命中 |
| L2: Workers KV | ✅ | 全球复制缓存，papernow_kv namespace |
| 缓存策略 | ✅ | 过去范围 7 天，当前范围 1 小时 |
| X-Cache 头 | ✅ | L1-HIT / L2-HIT / MISS |

**效果**: 首次搜索 ~10s，缓存命中后 <1s

### O6: SPA 刷新修复 — ✅ 完成

| 子任务 | 状态 | 说明 |
|--------|------|------|
| Pages Function SPA fallback | ✅ | functions/[[path]].ts 返回 index.html + 200 |
| 尾部斜杠 URL 修复 | ✅ | 客户端 history.replaceState 修正路径 |
| 404.html 生成 | ✅ | Vite 插件 post-build 复制 |
| _redirects 简化 | ✅ | 移除 Netlify :splat 语法 |

### O7: 部署类型错误修复 — ✅ 完成

- 添加 `declare const caches: CacheStorage & { default: Cache }`
- 添加 `interface KVNamespace` 内联类型声明

### O8: UI 优化 — ✅ 完成

| 子任务 | 状态 | 说明 |
|--------|------|------|
| 长链接截断 | ✅ | DOI/原文链接 2 行截断 + ellipsis |
| 底部导航 Refresh | ✅ | Search → Refresh，onTabItemTap 触发 location.reload() |
